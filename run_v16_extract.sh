#!/bin/bash
# bench_v16 extraction: 1 region x 3 depths, each run exactly once.
# Coordinates come from results/bench_v16/region_selection.json. Same extractor,
# same flags and the same resume guard as run_v14_extract.sh / run_v15_extract.sh,
# so the v16 test cells are produced by byte-identical code to every earlier cell.
#
...
# leaves nothing in cache/bench_v16/ to resume from, and the guard is safe.
set -u
D=data/giab_hg002_v16
mkdir -p cache/bench_v16 logs_v16

run () {
  local name=$1 contig=$2 start=$3 stop=$4 tag=$5
  local key="${name}_${tag}"
  if [ -s "cache/bench_v16/${key}.npz" ]; then
    echo "SKIP $key (already extracted) $(date -Is)"
    return 0
  fi
  echo "START $key $(date -Is)"
  local scratch="cache/bench_v16/${key}.partial.npz"
  rm -f "$scratch"
  if python3 extract_bench_v12.py \
      ...
      --features none \
      --out "$scratch" > "logs_v16/${key}.log" 2>&1; then
    mv "$scratch" "cache/bench_v16/${key}.npz"
...
for tag in full 30x 15x; do
  run chr13_representative chr13 70000000 73000000 $tag &
done
wait
echo "ALL EXTRACTED $(date -Is)"
ls -l cache/bench_v16/
