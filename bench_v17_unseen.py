"""bench_v17 — the frozen cascade on the hardest unseen contig, chr17:18–21 Mb.

Protocol is pre-registered in ``History/17_DEVLOG.md`` §1–§10 and is not
...
Three cells are scored — one 3 Mb window on chr17, a contig no experiment in this
project has touched, at native / 30x / 15x depth. The window was chosen by
``select_v17_region.py`` from public annotation alone, before any caller ran and
before the chr17 FASTA existed. Unlike devlog 16's median-difficulty chr13
window, Rule C17/R17 deliberately took the *hardest* remaining contig and its
most segdup-saturated scorable window: 36.6% of its high-confidence bases sit in
``lowmap_segdup`` and 45.1% are annotated ``alldifficult``.

Structurally this file is ``bench_v16_unseen.py`` with the region, the data
directories and the exclusion list advanced. Every metric helper, the bootstrap,
the controls, the sweep and the disagreement machinery are imported unchanged
from v12/v13/v14, so the v17 numbers are produced by byte-identical scoring code
to every earlier devlog.
"""
...
#: Must match what ``run_v17_extract.sh`` used (``extract_bench_v12.CHUNK_BP``),
...
BASE_REGIONS = {
    "chr17_segdup": {
        "contig": "chr17", "start": 18_000_000, "stop": 21_000_000,
        "regime": "segmental-duplication-saturated chr17 17p11.2 (hardest unused "
                  "contig by segdup_hc_fraction, hardest scorable window on it)",
        "gc": 0.46275733333333335,
    },
}
...
DATA_DIR = Path("data/giab_hg002_v17")
CACHE_DIR = Path("cache/bench_v17")
OUT_DIR = Path("results/bench_v17")
