...
struct TaskResult {
    bool done = false;
    std::string vcf_cascade, vcf_pbonly, routed_tsv, dump_tsv;
...
struct Shared {
    ...
    size_t next_flush = 0;
    // outputs
    FILE *f_casc = nullptr, *f_pbo = nullptr, *f_routed = nullptr, *f_dump = nullptr, *f_bin = nullptr, *f_pbb = nullptr;
...
std::string process_task(Shared& S, const Task& t, BamHandle& bam, Fai* fai, std::vector<RowEntry>& scratch, TaskResult& R) {
    ...
    if (!cfg.exact_windows) {
        ...
        auto col = [](void* v, int64_t pos, const bam_pileup1_t* pil, int n_plp) {
            ...
            const bool need = f.cfg->mode == Mode::PB_ALL || (*f.routed)[i] || (*f.forced)(i);
            if (!need) return;
            int n = 0, keep;
            const RowEntry* kept;
...
std::string run(const Config& cfg, Summary& out) {
    ...
    for (FILE* f : {S.f_casc, S.f_pbo, S.f_routed, S.f_dump, S.f_bin, S.f_pbb}) if (f) std::fclose(f);
...
std::string summary_json(const Config& c, const Summary& s) {
    ...
    std::snprintf(b, sizeof b,
        ...
        (long long)s.n_windows, (long long)s.n_loci, (long long)s.n_frame, (long long)s.n_routed, (long long)s.n_pb_evaluated,
