"""bench_v18 — CROSS-SAMPLE generalization test: the frozen cascade on HG003.

Protocol is pre-registered in ``History/18_DEVLOG.md`` and is not re-derived
here. Nothing in this file fits, selects or modifies a threshold: the three
frozen constants are imported from ``cascade`` and only ever read, and the
cutoff sweep exists solely to draw the diagnostic accuracy-vs-compute curve.

This is the deliberate opposite of every prior devlog's independence axis. All
of v13-v17 varied the *region* while holding the *sample* fixed at HG002, and
their audits therefore check for coordinate/contig reuse. This experiment does
the reverse: it reuses devlog 16's exact chr13:70,000,000-73,000,000 window
*verbatim*, on the same platform (Illumina 2x250) and the same aligner
(novoalign), and changes only the sample, HG002 -> HG003 (trio father). Holding
region, platform and aligner fixed isolates the sample variable, which is the
one axis this project has never tested. The independence question here is not
"was this coordinate range touched before" (it deliberately was) but "was this
*sample's* data, or any HG003-conditioned threshold, used anywhere before this
run" -- answered by ``verify_caches`` below, which checks the BAM/VCF source
paths are HG003, that no frozen constant differs from ``cascade.py``, and that
the git-tracked frozen files are byte-identical to their v17 hashes.

Three cells are scored -- native / 30x / 15x depth on the HG003 realisation of
the same 3 Mb window. Every metric helper, the bootstrap, the controls, the
sweep and the disagreement machinery are imported unchanged from v12/v13/v14,
so the v18 numbers are produced by byte-identical scoring code to every earlier
devlog; only the input BAM/VCF/BED (HG003's own) differ.
"""
...
#: Must match what ``run_v18_extract.sh used (``extract_bench_v12.CHUNK_BP``),
#: because the window tiling — and therefore which loci exist — is rebuilt per
#: chunk.
CHUNK_BP = 500_000
SEQ_LEN = 64

#: The one region, reused verbatim from devlog 16 §4.3 (HG002), now realised on
#: HG003. Region, platform and aligner are held fixed; only the sample changes.
BASE_REGIONS = {
    "chr13_crosssample": {
        "contig": "chr13", "start": 70_000_000, "stop": 73_000_000,
        "regime": "devlog-16 median-difficulty chr13 window, HG002 -> HG003 "
                  "sample swap; region/platform/aligner held fixed",
        "gc": 0.35249166666666665,
    },
}
DEPTH_TAGS = ("full", "30x", "15x")

#: This experiment intentionally reuses the devlog-16 coordinates on a
#: different sample, so contig/coordinate reuse is not a leakage signal here.
#: Left empty so ``verify_caches`` never flags the deliberate reuse as overlap;
#: the real independence checks are ``expected_sample`` and ``expected_bam``
#: below.
PREVIOUSLY_USED_CONTIGS: set[str] = set()
EXPECTED_SAMPLE = "HG003"
EXPECTED_BAM_TOKEN = "HG003.GRCh38.2x250"

DATA_DIR = Path("data/giab_hg002_v18")
CACHE_DIR = Path("cache/bench_v18")
OUT_DIR = Path("results/bench_v18")
...
def _bam_sample_is_hg003(bam_path: str) -> bool:
    """True sample-identity check.

    The upstream GIAB BAMs carry no ``@RG SM`` tag (checked directly: neither
    the HG002 nor the HG003 novoalign BAM has one), so BAM-header identity is
    not available. Two independent signals are used instead, both anchored to
    *this specific extraction*, not to a filename convention:

    1. The truth VCF's own sample column, read via ``bcftools query -l`` on
       the exact VCF this cell scores against -- this is the authoritative
       GIAB sample identity and cannot silently drift from what genotypes are
       actually being compared.
    2. The BAM's own ``@PG`` provenance line, written by ``samtools view``
       itself at slice time, which embeds the source URL the slice was cut
       from (recorded automatically, not asserted by this script).

    Fails closed: either signal missing or not naming HG003 marks the cell
    ``NOT ok``.
    """
    import subprocess
    ...
    vcf_path = str(Path(bam_path).parent / "chr13_crosssample.vcf.gz")
    try:
        out = subprocess.run(["bcftools", "query", "-l", vcf_path],
                             capture_output=True, text=True, check=True)
        vcf_sample_ok = out.stdout.strip() == EXPECTED_SAMPLE
    except Exception:
        vcf_sample_ok = False
    ...
    try:
        ...
        bam_provenance_ok = any(EXPECTED_BAM_TOKEN in pg.get("CL", "") for pg in pg_lines)
    except Exception:
        bam_provenance_ok = False

    return bool(vcf_sample_ok and bam_provenance_ok)
...
def verify_caches(loaded: dict[str, dict]) -> dict:
    ...
    for key, region in loaded.items():
        ...
        entry = {
            ...
            # Deliberately reused verbatim from devlog 16 (HG002) on a
            # different sample (HG003); disclosed, not a leakage signal here.
            "coordinate_overlaps_historical_regions_disclosed": overlaps,
            "bam": region["bam"],
            "sample_is_hg003": _bam_sample_is_hg003(region["bam"]),
        }
        entry["ok"] = bool(
...
def mapq_stratified(region: dict) -> dict:
    ...
    mapping-quality one, and devlog 17 §6 lists mapping quality among the axes to
    report.
    ...
    — and it is inherited unchanged from devlog 16 §17.
    """
