"""Render the bench_v17 results JSON as the markdown tables devlog 17 needs.
...
RESULTS = Path("results/bench_v17/benchmark_results.json")
DISAGREEMENTS = Path("results/bench_v17/disagreements.json")

v14.ORDER = ["chr17_segdup"]
...
def stratum_table(report: dict, axis: str, title: str) -> str:
    lines = [f"**{title}**", "",
             ...
             "|---|---|---:|---:|---:|---:|---:|---:|---:|"]
    for tag in v14.TAGS:
        cell = report["cells"].get(f"chr17_segdup_{tag}")
        if cell is None:
            continue
...
def disagreement_detail() -> str:
    ...
    for key, entry in data.items():
        for record in entry["records"]:
            lines.append(
                f"| {key.replace('chr17_segdup_', '')} | {record['position']:,} | "
