"""Render the bench_v18 results JSON as the markdown tables devlog 17 needs.
...
``summarize_v14``'s, re-pointed at the v17 artefacts — only the generic
stratum table (VAF, base quality, mapping quality, Stage-1 hard strata) is new,
because devlog 17 §6 reports axes devlog 14 stored but never rendered.
"""
...
RESULTS = Path("results/bench_v18/benchmark_results.json")
DISAGREEMENTS = Path("results/bench_v18/disagreements.json")

v14.ORDER = ["chr13_crosssample"]
...
def stratum_table(report: dict, axis: str, title: str) -> str:
    lines = [f"**{title}**", "",
             ...
             "|---|---|---:|---:|---:|---:|---:|---:|---:|"]
    for tag in v14.TAGS:
        cell = report["cells"].get(f"chr13_crosssample_{tag}")
        if cell is None:
            continue
...
def disagreement_detail() -> str:
    ...
    for key, entry in data.items():
        for record in entry["records"]:
            lines.append(
                f"| {key.replace('chr13_crosssample_', '')} | {record['position']:,} | "
...
def main() -> None:
    ...
    for axis, title in (("by_vaf", "VAF-stratified"),
                        ...
                        ("by_mapq", "Mapping-quality-stratified"),
