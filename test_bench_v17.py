"""Tests for the bench_v17 hardest-unseen-region benchmark driver.

The v14 suite already covers the shared helpers this driver imports. What is
tested here is only what v17 adds, and it is the set of things that could
...
import bench_v17_unseen as bench
...
def test_driver_never_assigns_to_a_frozen_constant():
    source = Path("bench_v17_unseen.py").read_text()
...
def test_region_matches_the_results_blind_selection():
    """The driver's hard-coded region must equal what select_v17_region.py chose."""
    import json
    chosen = json.loads(Path("results/bench_v17/region_selection.json").read_text())["selected_window"]
    spec = bench.BASE_REGIONS["chr17_segdup"]
    assert (spec["contig"], spec["start"], spec["stop"]) == (
        chosen["contig"], chosen["start"], chosen["stop"])
    assert spec["gc"] == pytest.approx(chosen["gc_fraction"])
...
def _cell(n_loci: int = 128, contig: str = "chr17", start: int = 18_000_000):
    ...
    return {
        ...
        "spec": {"contig": contig, "start": start, "stop": start + n_loci,
                 "base": "chr17_segdup", "depth_tag": "full", "gc": 0.4628},
    }
...
def test_audit_flags_a_coordinate_overlap_with_a_historical_region():
    ...
    cell = _cell(contig="chr17", start=18_000_000)
    cell["spec"] = {**cell["spec"], "contig": "chr17"}
    bench.HISTORICAL_REGIONS.append(("chr17", 18_000_000, 18_001_000))
    try:
        audit = bench.verify_caches({"c": cell})
        assert audit["all_ok"] is False
        assert audit["cells"]["c"]["coordinate_overlaps_historical_regions"]
